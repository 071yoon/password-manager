...
const inputGroupVariants = cva('inline-flex items-stretch w-full', {
  variants: {
    variant: {
      default: 'bg-white/95 dark:bg-slate-800/95',
...
const inputGroupButtonVariants = cva(
  'inline-flex shrink-0 items-center justify-center border border-slate-300/90 text-slate-700 transition dark:border-slate-600 dark:text-slate-100',
  {
    variants: {
      variant: {
        default: 'bg-white/95 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700',
  ...
  ({ className, ...props }, ref) => (
    <input
      ...
      className={cn(
        ...
        'dark:border-slate-600 dark:bg-slate-800/95 dark:text-slate-100 dark:focus:border-slate-500 dark:focus:ring-slate-700',
        className,
      )}
      {...props}
    />
